...
make_data_matrices <- function(data_quarterly, data_monthly = NULL, data_weekly = NULL, data_daily = NULL){
  ...
  if(!is.null(data_weekly)){
    ...
    colnames(Y_W) = paste(rep(seriesW_names, each = m2), rep(c(paste0("W", m2:1), m2)))
  ...
  if(!is.null(data_daily)){
    ...
    colnames(Y_D) = paste(rep(seriesD_names, each = m3), rep(c(paste0("D", m3:1), m3)))
...
inputs_sparseMFVAR <- function(k_q, k_m, k_w, k_d,
  ...
  B_matrix_1 = construct_B_matrix(k_q = k_q, k_m = k_m, k_w = k_w, k_d = k_d)
  ...
  penalty = penalty_function(k_q = k_q, k_m = k_m, k_w = k_w, k_d = k_d, 
  ...
  columns_matrix = columns_function(k_q = k_q, k_m = k_m, k_w = k_w, k_d = k_d)
...
# Construction of B matrix 
construct_B_matrix <- function(k_q = 1, k_m = 1, k_w = 0, k_d = 0){
...
columns_function <- function(k_q = 1, k_m = 1, k_w = 0, k_d = 0){
  ...
  if(k_q != 0 & k_m != 0 & k_w == 0 & k_d == 0){
    ...
    for(l in 1:k_q){
      for(j in 1:k_m){
        columns_matrix[i,1:m1] = c((K*(l-1)+k_q+2*j+(j-2)):(K*(l-1)+k_q+2*j+(j-2)+2))
        i = i+1
      }
    }
    ...
    for(l in 1:k_m){
      for(j in 1:k_q){
        columns_matrix[i,1:m1] = c(K*(k_q+(l-1)*3)+ j, K*(k_q+(l-1)*3+1)+ j, K*(k_q+(l-1)*3+2)+ j)
        i = i+1
      }
    }
    ...
    for(l in 1:k_m){
      for(j in 1:k_m){
        columns_matrix[i,1:m1^2] = c((K*(k_q+(l-1)*3)+k_q+2*j+(j-2)):(K*(k_q+(l-1)*3)+k_q+2*j+(j-2)+2),
  ...
  if(k_q != 0 & k_m == 0 & k_w != 0 & k_d == 0){
    ...
    for(l in 1:k_q){
      for(j in 1:k_w){
        columns_matrix[i,1:m2] = c((K*(l-1)+k_q+m1*k_m+m2*(j-1)+1):(K*(l-1)+k_q+m1*k_m+m2*(j-1)+12))
        i = i+1
      }
    }
    ...
    for(l in 1:k_w){
      for(j in 1:k_q){
        columns_matrix[i,1:m2] = K*(k_q+m1*k_m+(l-1)*12+ c(0:11)) +j 
    ...
    for(l in 1:k_w){
      for(j in 1:k_w){
        columns_matrix[i,1:(m2^2)] = c((K*(k_q+m1*k_m+(l-1)*12)+ k_q+m1*k_m+m2*(j-1)+1) : (K*(k_q+m1*k_m+(l-1)*12)+ k_q+m1*k_m+m2*(j-1)+12) , 
  ...
  if(k_q != 0 & k_m == 0 & k_w == 0 & k_d != 0){
    ...
    for(l in 1:k_q){
      for(j in 1:k_d){
        columns_matrix[i,1:m3] = c((K*(l-1)+k_q+m1*k_m+m2*k_w+m3*(j-1)+1):(K*(l-1)+k_q+m1*k_m+m2*k_w+m3*(j-1)+60))
        i = i+1
      }
    }
    ...
    for(l in 1:k_d){
      for(j in 1:k_q){
        columns_matrix[i,1:m3] = K*(k_q+m1*k_m+m2*k_w+(l-1)*60+c(0:59))+ j
        i = i+1
      }
    }
    ...
    for(l in 1:k_d){
      for(j in 1:k_d){                                                                
        ...
        columns_matrix[i,1:(m3^2)] = sort(b)
        i = i+1
        b = c()
      }
    }
  ...
  if(k_q != 0 & k_m != 0 & k_w != 0 & k_d == 0){
    ...
    for(l in 1:k_m){
      for(j in 1:k_w){
        columns_matrix[i,1:(m1*m2)] = c((K*(k_q+(l-1)*3)+k_q+m1*k_m+m2*(j-1)+1):(K*(k_q+(l-1)*3)+k_q+m1*k_m+m2*(j-1)+12),
    ...
    for(l in 1:k_w){
      for(j in 1:k_m){
        columns_matrix[i,1:(m1*m2)] = c((K*(k_q+m1*k_m+(l-1)*12)+ k_q+2*j+(j-2)) : (K*(k_q+m1*k_m+(l-1)*12)+ k_q+2*j+(j-2)+2) , 
  ...
  if(k_q != 0 & k_m != 0 & k_w == 0 & k_d != 0){
    ...
    for(l in 1:k_m){
      for(j in 1:k_d){
        columns_matrix[i,1:(m1*m3)] = c((K*(k_q+(l-1)*3)+k_q+m1*k_m+m2*k_w+m3*(j-1)+1):(K*(k_q+(l-1)*3)+k_q+m1*k_m+m2*k_w+m3*(j-1)+60),
    ...
    for(l in 1:k_d){
      for(j in 1:k_m){
        ...
        columns_matrix[i,1:(m1*m3)] = sort(c(help_1,help_2,help_3))
        i = i+1
      }
    }
  ...
  if(k_q != 0 & k_m == 0 & k_w != 0 & k_d != 0){
    ...
    for(l in 1:k_w){
      for(j in 1:k_d){
        columns_matrix[i,1:(m2*m3)] = c((K*(k_q+m1*k_m+(l-1)*12)+ k_q+m1*k_m+m2*k_w+m3*(j-1)+1) : (K*(k_q+m1*k_m+(l-1)*12)+ k_q+m1*k_m+m2*k_w+m3*(j-1)+60) , 
    ...
    for(l in 1:k_d){
      for(j in 1:k_w){
        ...
        columns_matrix[i,1:(m2*m3)] = sort(c(help_1,help_2,help_3, help_4, help_5, help_6, help_7, help_8, help_9, help_10, help_11, help_12))
...
hierarchy_indices = function(penalty, row = 1, column = 1){
...
penalty_function = function(k_q = 1, k_m = 1, k_w = 0, k_d = 0, 
  ...
  if(k_q != 0 & k_m != 0 & k_w == 0 & k_d == 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "L1" & penalty_own_on_own == "HIER"){
      ...
      for(i in 1:(k_q^2)){
        penalty_matrix[i, 1] = hierarchy_indices(penalty = 3, row = 1, column = 1) #phi
      }
      for(i in (2*k_m*k_q+k_q^2+1):(k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:m1^2] = hierarchy_indices(penalty = 3, row = 3, column = 3) #rho
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      ...
      for(i in (k_m*k_q+k_q^2+1):(2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:m1] = hierarchy_indices(penalty = 3, row = 3, column = 1) #theta
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      ...
      for(i in (k_q^2+1):(k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:m1] = hierarchy_indices(penalty = 2, row = 1, column = 3) #pi
      }
    }
  ...
  if(k_q != 0 & k_m == 0 & k_w != 0 & k_d == 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "L1" & penalty_own_on_own == "HIER"){
      ...
      for(i in (2*k_q*k_w+k_q^2+1):(k_w^2+2*k_q*k_w+k_q^2)){
        penalty_matrix[i, 1:m2^2] = hierarchy_indices(penalty = 3, row = 12, column = 12) #gamma
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in  (k_q*k_w+k_q^2+1):(2*k_q*k_w+k_q^2)){
        penalty_matrix[i, 1:m2] = hierarchy_indices(penalty = 3, row = 12, column = 1) #mu
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q^2+1):(k_q*k_w+k_q^2)){
        penalty_matrix[i, 1:m2] = hierarchy_indices(penalty = 2, row = 1, column = 12) #alpha
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "HIER"){
      penalty_indicator = rep(1,num_groups) 
  ...
  if(k_q != 0 & k_m == 0 & k_w == 0 & k_d != 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "L1" & penalty_own_on_own == "HIER"){
      ...
      for(i in (2*k_q*k_d+k_q^2+1):(k_d^2+2*k_q*k_d+k_q^2)){
        penalty_matrix[i, 1:m3^2] = hierarchy_indices(penalty = 3, row = 60, column = 60) #kappa
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q*k_d+k_q^2+1):(2*k_q*k_d+k_q^2)){
        penalty_matrix[i, 1:m3] = hierarchy_indices(penalty = 3, row = 60, column = 1) #eta
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q^2+1):(k_q*k_d+k_q^2)){
        penalty_matrix[i, 1:m3] = hierarchy_indices(penalty = 2, row = 1, column = 60) #epsilon
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "HIER"){
      penalty_indicator = rep(1,num_groups) 
  ...
  if(k_q != 0 & k_m != 0 & k_w != 0 & k_d == 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      ...
      for(i in (2*k_q*k_w+k_m*k_w+k_m^2+2*k_m*k_q+k_q^2+1):(2*k_m*k_w+2*k_q*k_w+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:(m1*m2)] = hierarchy_indices(penalty = 3, row = 12, column = 3) #lambda
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q*k_w+k_m^2+2*k_m*k_q+k_q^2+1):(k_m*k_w+k_q*k_w+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:(m1*m2)] = hierarchy_indices(penalty = 3, row = 3, column = 12) #beta
      }
    }
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "HIER"){
      penalty_indicator = rep(1,num_groups) 
  ...
  if(k_q != 0 & k_m != 0 & k_w == 0 & k_d != 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      ...
      for(i in (2*k_q*k_d+k_m*k_d+k_m^2+2*k_m*k_q+k_q^2+1):(2*k_m*k_d+2*k_q*k_d+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:(m1*m3)] = hierarchy_indices(penalty = 3, row = 60, column = 3) #tau
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q*k_d+k_m^2+2*k_m*k_q+k_q^2+1): (k_m*k_d+k_q*k_d+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:(m1*m3)] = hierarchy_indices(penalty = 3, row = 3, column = 60) #delta
      }
  ...
  if(k_q != 0 & k_m == 0 & k_w != 0 & k_d != 0){
    ...
    if(penalty_H_on_L == "L1" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      ...
      for(i in (2*k_q*k_d+k_w*k_d+k_w^2+2*k_q*k_w+k_q^2+1):(2*k_w*k_d+2*k_q*k_d+k_w^2+2*k_q*k_w+k_q^2)){
        penalty_matrix[i, 1:(m2*m3)] = hierarchy_indices(penalty = 3, row = 60, column = 12) #nu
      }
    }
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "L1" & penalty_own_on_own == "L1"){
      penalty_indicator = rep(1,num_groups) 
      ...
      for(i in (k_q*k_d+k_w^2+2*k_q*k_w+k_q^2+1):(k_w*k_d+k_q*k_d+k_w^2+2*k_q*k_w+k_q^2)){
        penalty_matrix[i, 1:(m2*m3)] = hierarchy_indices(penalty = 3, row = 12, column = 60) #xi
      }
  ...
  if(k_q != 0 & k_m != 0 & k_w != 0 & k_d != 0){
    ...
    if(penalty_H_on_L == "HIER" & penalty_L_on_H == "HIER" & penalty_own_on_own == "L1"){
      ...
      for(i in (2*k_d*k_q+k_w*k_d+k_m*k_d+k_w^2+2*k_m*k_w+2*k_q*k_w+k_m^2+2*k_m*k_q+k_q^2+1):(2*k_m*k_d+2*k_d*k_q+k_w*k_d+k_w^2+2*k_m*k_w+2*k_q*k_w+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:m1] = hierarchy_indices(penalty = 3, row = 60, column = 3) 
      }
      for(i in (2*k_m*k_d+2*k_d*k_q+k_w*k_d+k_w^2+2*k_m*k_w+2*k_q*k_w+k_m^2+2*k_m*k_q+k_q^2+1):(2*k_w*k_d+2*k_m*k_d+2*k_d*k_q+k_w^2+2*k_m*k_w+2*k_q*k_w+k_m^2+2*k_m*k_q+k_q^2)){
        penalty_matrix[i, 1:(m2*m3)] = hierarchy_indices(penalty = 3, row = 60, column = 12) #nu
      }
